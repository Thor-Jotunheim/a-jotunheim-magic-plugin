...
class UnifiedTeller {
    ...
    clearCart() {
        ...
        if (customerNameInput) {
            ...
            console.log('🚨 DEBUG: Customer name field cleared');
        }
        
        // Clear customer validation and data
        this.currentCustomer = null;
        this.hideValidationIcon();
        console.log('🚨 DEBUG: Customer validation cleared');
        
        // Clear Recent Transaction section
        ...
        if (container) {
            ...
            if (shopType && shopType.shopType === 'aesir') {
                ...
                container.innerHTML = '<div class="transaction-item">No transactions to display</div>';
            }
            console.log('🚨 DEBUG: Recent Transaction section cleared');
        }
    ...
    handleQuantityBlur(inputElement) {
        ...
        // If the field is empty or invalid, restore appropriate default value
        if (inputElement.value === '' || isNaN(parseInt(inputElement.value))) {
            // Turn-in inputs default to "0", buy/sell inputs default to "1"
            if (inputElement.id.includes('turnin-')) {
                inputElement.value = '0';
            } else if (inputElement.id.includes('buy-') || inputElement.id.includes('sell-')) {
                inputElement.value = '1';
            } else {
                inputElement.value = '0'; // Default fallback
            }
        } else {
            inputElement.value = value;
        }
    ...
    handleQuantityFocus(inputElement) {
        // Clear the field if it contains default values to make typing easier
        // Different input types have different default values:
        // - Turn-in inputs default to "0"
        // - Buy/sell inputs default to "1" 
    ...
    createTableRow(item) {
        ...
        unitsCell.className = 'units-cell';
        unitsCell.innerHTML = `
            <div class="units-section">
                ${this.generateUnitsControls(item, isTurnInItem)}
            </div>
        `;
