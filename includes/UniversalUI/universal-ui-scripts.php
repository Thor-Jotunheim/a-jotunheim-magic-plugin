...
function jotunheim_enqueue_universal_ui_scripts() {
    ...
    selectedRecords.forEach(recordId => {
        ...
        .then(record => {
            ...
            const formHtml = universalGenerateEditForm(record, columns, table); // Pass the table name here
            editContainer.insertAdjacentHTML('beforeend', formHtml);
            if (table === 'jotun_eventzones') {
                universalInitializeFieldBehavior(); // Initialize custom field behaviors
            }
        })
    ...
    recordsContainer.insertAdjacentHTML('beforeend', checkbox);
        ...
        function universalGenerateEditForm(record, columns, table) {
    const booleanFields = [
...
formHtml += `<h4>Editing: ${displayName}</h4>`;
    ...
    columns.forEach(column => {
        ...
        if (['id', 'string_name'].includes(field_name)) return;  // Skip ID fields
        ...
        // Custom Fields for jotun_eventzones Table
        if (table === 'jotun_eventzones') {
            if (field_name === 'shape') {
            ...
            formHtml += `<input type='text' id='${field_name}' name='${field_name}' value='${record[field_name] || ''}' style='padding: 10px; border-radius: 5px; border: 2px solid #ccc; width: 100%;'>`;
        }
